...
from TTS.Dia_audio import synthesize_dia_audio
...
def run_dia_pipeline():
    print(f"--- Starting NLP + Dia TTS pipeline using Ollama model: {OLLAMA_MODEL_NAME} ---")

    dia_nlp_output_subdir = os.path.join(NLP_OUTPUT_DIR, "dia_nlp_texts")
    os.makedirs(dia_nlp_output_subdir, exist_ok=True)
    ...
    for filename in os.listdir(OCR_INPUT_DIR):
        if filename.endswith(".txt"):
            ...
            try:
                ...
                print(f"Generating NLP lecture for Dia from {filename}...")
                lecture_text = generate_professor_lecture(
                    notes=ocr_notes,
                    ollama_model_name=OLLAMA_MODEL_NAME,
                    system_prompt_type="dia"
                )
                ...
                nlp_output_filename = f"{os.path.splitext(filename)[0]}_dia_nlp.txt"
                nlp_output_filepath = os.path.join(dia_nlp_output_subdir, nlp_output_filename)
                with open(nlp_output_filepath, "w", encoding="utf-8") as f:
                    f.write(lecture_text)
                print(f"NLP output saved to: {nlp_output_filepath}")

                audio_output_filename = f"{os.path.splitext(filename)[0]}_dia.wav"
                print(f"Synthesizing audio with Dia for {audio_output_filename}...")
                saved_audio_path = synthesize_dia_audio(
                    text=lecture_text,
